...
Najlepsze rozwiązanie dla grafu pełnego wraz z wartością funkcji celu zostanie zapamiętane. Reprezentująca je ścieżka pozostanie niezmieniona, podczas gdy w kolejnych testach poruszanie się po określonej części ulic będzie niedozwolone. Koszt niedozwolonych połączeń będzie zwiększony o umowną wartość nieskończoności - 1e38.

Następnym krokiem będzie wygenerowanie macierzy kosztów $C$ wraz z losowymi ulicami jednokierunkowymi. Wagi krawędzi zostaną dostosowane tak, by zapewnić istnienie cyklu Hamiltona w grafie. Liczba ulic jednokierunkowych, jak i liczba miast będzie się zmienna w zależności od wygenerowanych danych. Pozwoli to zbadać zadania dużej skali (ok. 1000 wierzchołków). Wprowadzana liczba ulic jednokierunkowych będzie pewnym przyjętym procentem wszystkich ulic istniejących w grafie.

Wyniki zostaną przedstawione w postaci tabeli. Dla określonej liczby miast i określonego procentu niedozwolonych połączeń, podany zostanie procent prawidłowych rozwiązań. Każdy test zostanie powtórzony kilka razy.
